 ...
 * @param fileName target fileName (without file-extension), required for the index.js generation
 */
export function generateTS(
  SD: SQSD.sdTemplate,
  mashupLogic: SQSD.mashupLogic,
  fileName: string
): { codeTS: string; baseTS: string } {
  const varPrefix = 'mavar'
  const propPrefix = 'mapro'
  let customCount = 0
  const customDataPush = {}
  let customDataPushCount = 0
  const baseTS = generateIndexJS(SD, fileName)
  const filestart = `
"use strict"
import * as WoT from 'wot-typescript-definitions';
const { hooks } = require('@feathersjs/hooks');

// FIXME: cannot require app (lives in typescript file)
// FIXME: const app = require(__dirname + '/src/app');

post({ name: 'hostOS', op: 'writeproperty', data: process.platform })

setInterval(() => {
  post({ name: 'memoryUsage', op: 'writeproperty', data: process.memoryUsage() })
  post({ name: 'resourceUsage', op: 'writeproperty', data: process.resourceUsage() })
  post({ name: 'uptime', op: 'writeproperty', data: process.uptime() })
}, 5000)
...
`
  ...
  const codeTS = beautify(filestart + generateClassWotMashup().join('\n'), {
  ...
  function generateClassWotMashup() {
    ...
    Object.getOwnPropertyNames(SD.variables).forEach((v) => {
      jsOut.push('private ' + varPrefix + v + '\n')
    })
    Object.getOwnPropertyNames(SD.properties).forEach((v) => {
      jsOut.push('private ' + propPrefix + v + '\n')
    })
  ...
  function genAppLogic() {
    ...
    if (mashupLogic.root) {
      out.push(`private execMashupLogic() {
                            post({ name: 'step', op: 'writeproperty', data: 'private:execMashupLogic' })
    ...
    Object.keys(mashupLogic.properties).forEach((prop) => {
      out.push(`private get_${prop}() {
                            post({ name: 'step', op: 'writeproperty', data: 'private:get_${prop}' })
                            return new Promise<any>(async (resolve, reject) => {
    ...
    Object.keys(mashupLogic.actions).forEach((prop) => {
      out.push(`private act_${prop}() {
                            post({ name: 'step', op: 'writeproperty', data: 'private:act_${prop}' })
                            return new Promise<any>(async (resolve, reject) => {
    ...
    Object.keys(mashupLogic.functions).forEach((prop) => {
      out.push(`private func_${prop}() {
                            post({ name: 'step', op: 'writeproperty', data: 'private:func_${prop}' })
                            return new Promise<any>(async (resolve, reject) => {
  ...
  function genClassConstructor() {
    ...
    Object.keys(SD.things).forEach((tdTitle) => {
      ...
      currentTd.title = currentTd.title ?? currentTd.$id.slice(1)
    ...
    out.push(`
                    ...
                    post({ name: 'step', op: 'writeproperty', data: 'consume-tds' })
    ...
    out.push(`  this.WoT.produce(
                        ...
                        post({ name: 'step', op: 'writeproperty', data: 'produced-thing' })
                        this.thing = exposedThing
                        this.td = exposedThing.getThingDescription()
                        ${
                          Object.keys(SD.properties).length > 0
                            ? 'this.add_properties()'
                            : ''
                        }
                        ${
                          Object.keys(SD.actions).some(
                            (prop) => SD.actions[prop].path !== undefined
                          )
                            ? 'this.add_actions()'
                            : ''
                        }
  ...
  function genAddProperties() {
    ...
    Object.keys(SD.properties).forEach((prop) => {
      if (SD.properties[prop].defaultInput !== undefined) {
        out += `this.thing.writeProperty("${prop}", ${genDefaultString(
          SD.properties[prop].defaultInput
        )})\n`
      }
      ...
      if (SD.properties[prop].path !== undefined) {
        ...
        out += `this.thing.setPropertyReadHandler("${prop}", () => {
                    post({ name: 'step', op: 'writeproperty', data: 'propertyReadHandler:${prop}' })
                    return new Promise<any>((resolve, reject) => {
                        resolve(${composeVarName({
                          type: 'property',
                          name: prop
                        })})
                    })
  ...
  function genAddActions() {
      ...
      if (SD.actions[prop].path !== undefined) {
        out += `
                    this.thing.setActionHandler("${prop}", inputData => {
                        post({ name: 'step', op: 'writeproperty', data: 'actionHandler:${prop}' })
    ...
    out = Object.keys(SD.actions).some(
      (prop) => SD.actions[prop].path !== undefined
    )
      ? out
      : ''
  ...
  function parsePath(inPath: SQSD.structureEl[]) {
    let out = '// ### path: ###\n'
      ...
      if (nextEl.type === structureType.interact) {
        ...
        if (nextEl.loopOpts.type === loopType.logic) {
          if (nextEl.loopOpts.exCount === 'forever') {
            out += `while(${
              nextEl.loopOpts.exCount === 'forever'
                ? 'true'
                : nextEl.loopOpts.exCount
            })`
          } else {
        ...
        if (nextEl.get) {
          ...
          if (nextEl.defaultInput !== undefined) {
            out += ' ?? ' + genDefaultString(nextEl.defaultInput)
          }
          out += '\n'
        } else if (nextEl.defaultInput !== undefined) {
          out +=
            composeVarName(nextEl.set) +
            ' = ' +
            genDefaultString(nextEl.defaultInput) +
            '\n'
        } else {
        ...
        out +=
          'this.' +
          (nextEl.ref.type === 'action' ? 'act' : 'func') +
          '_' +
          nextEl.ref.name +
          '()\n'
      } else {
    ...
    function parsePathAtomic(inEl: SQSD.structElAtomic) {
      const syncs: string[] = []
      const waits: string[] = []
      const sends: string[] = []
      const asyncs: string[] = []
      ...
      const hasDataPush = inEl.receiveIntrcts.some(
        (rec) =>
          rec.type === interactionType.observe ||
          rec.type === interactionType.subscribe
      )
      if (hasDataPush) {
      ...
      inEl.sendIntrcts.forEach((snd) => {
        ...
        let inpost
        const fIntr =
          snd.type === interactionType.write ? 'writeProperty' : 'invokeAction'
        if (snd.get || snd.defaultInput !== undefined) {
          inpost =
            snd.get && snd.defaultInput !== undefined
              ? `${composeVarName(snd.get)} ?? ${genDefaultString(
                  snd.defaultInput
                )}`
              : snd.get
              ? composeVarName(snd.get)
              : snd.defaultInput !== undefined
              ? genDefaultString(snd.defaultInput)
              : ''
          sends.unshift(
            `const ${varname} = this.consumed_things["${snd.to}"].` +
              fIntr +
              `("${snd.name}", ${inname})`
          )
      ...
      inEl.receiveIntrcts.forEach((rec) => {
        ...
        if (
          rec.type === interactionType.read ||
          rec.type === interactionType.invoke
        ) {
          proto +=
            `${rec.set ? '' : 'let'} ${varname} = this.consumed_things["${
              rec.to
            }"].` +
            fIntr +
            `("${rec.name}")`
          ...
          if (rec.set) {
            hvar = 'autoGenReceive' + customCount++
          }
          customDataPush[customDataPushCount][dataPC] = false

          proto +=
            `this.consumed_things["${rec.to}"].` +
            fIntr +
            `("${rec.name}", async ${hvar} => {
      ...
      code = [pre, ...syncs, ...waits, ...asyncs, post].join('\n')
     ...
     */
    function parseIfCondition(cond: SQSD.comparison) {
      let cOut = ''

      if (cond.type === 'all') {
        cOut =
          '(' + cond.allOf.map((el) => parseIfCondition(el)).join(' && ') + ')'
      } else if (cond.type === 'one') {
        cOut =
          'this.oneOf(' +
          cond.oneOf.map((el) => parseIfCondition(el)).join(', ') +
          ')'
      } else if (cond.type === 'any') {
        cOut =
          '(' + cond.anyOf.map((el) => parseIfCondition(el)).join(' || ') + ')'
        ...
        throw new Error('problems with parseIfCondition')
      }
   ...
   */
  function genDefaultString(inDef: SQSD.typeDefaultInput) {
    let out = ''
   ...
   */
  function composeVarName(arg: {
    type: 'variable' | 'property'
    name: string
  }) {
    return (
      'this.' + (arg.type === 'property' ? propPrefix : varPrefix) + arg.name
    )
  }
}
 ...
 */
function generateIndexJS(SD: SQSD.sdTemplate, fileName: string) {
  // ########## config ###########
  ...
  Object.keys(SD.things).forEach((thingName) => {
    Object.keys(SD.things[thingName].actions).forEach((elName) => {
      ...
      if (formAr === undefined) {
        throw new Error('missing forms array in actions')
      }
      formAr.forEach((form) => {
    ...
    Object.keys(SD.things[thingName].properties).forEach((elName) => {
      ...
      if (formAr === undefined) {
        throw new Error('missing forms array in properties')
      }
      formAr.forEach((form) => {
        protocol = form.href.split('://').shift()
        if (protocol && protoClient.some((p) => p === protocol)) {
    ...
    Object.keys(SD.things[thingName].events).forEach((elName) => {
      ...
      if (formAr === undefined) {
        throw new Error('missing forms array in events')
      }
      formAr.forEach((form) => {
        protocol = form.href.split('://').shift()
        if (protocol && protoClient.some((p) => p === protocol)) {
